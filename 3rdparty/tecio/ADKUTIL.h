...
# endif
     ...
     * it is not converted to upper case).  Outer quotes are discarded if
     * retainOuterQuotes is FALSE. 
     *
     * @param CPtr
     *     Pointer into a zero terminated string of characters where the scanning
     *     is to begin. Upon return CPtr will either point to the end of the string
     ...
     *     Set to TRUE to retain the outer quotes.  Otherwise they are discarded.
     * @param Token
     ...
     * @param retainOuterQuotes
     *     Set to TRUE to retain the outer quotes for string values.  Otherwise 
     *     they are discarded.
     * @param ErrMsgString
    ...
    extern Boolean_t Macro_GetLgIndexArg(const char *Command,
                                         ...
                                         char      **ErrMsg);
    /**
     * Convenience function to fetch a variable assignment.  Variables can be
     * assigned using an integer or a string representing a variable name.
     *
     * @param Command
     *     Macro command name; this is only used for error messages.
     * @param ArgString
     *     Macro argument string containing a string representing the integer
     *     to fetch or a quoted string representing the variable name.
     * @param allowZero
     *     Some addons wish to allow zero as a legit assignment which means not assigned.
     * @param varNum
     *     Reference to EntIndex_t to return the variable number.
     * @param ErrMsg
     *     Allocated error string if not successful otherwise it is not used.
     *
     * @return
     ...
     *     allocated string describing the problem.
     */
    extern Boolean_t Macro_GetVarAssignment(const char* command,
                                            const char* argString,
                                            Boolean_t   allowZero,
                                            EntIndex_t& varNum,
                                            char**      errMsg);
